...
export default function DetailResepPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    if (!resepDetail) return <div className="min-h-screen flex items-center justify-center text-gray-500">Loading detail resep...</div>;

    return (
        <main className="min-h-screen bg-green-50 py-8 px-4 md:px-8" style={{ transform: "translateZ(0)" }}>
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-white rounded-2xl p-6 flex flex-col items-center justify-center shadow-sm border border-gray-100 gap-2">
                        ...
                        <span className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Waktu</span>
                        <span className="text-gray-800 font-bold">{resepDetail.waktu_memasak} menit</span>
                    </div>
                    <div className="bg-white rounded-2xl p-6 flex flex-col items-center justify-center shadow-sm border border-gray-100 gap-2">
                        <Users className="w-8 h-8 text-green-500 mb-1" />
                        <span className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Porsi</span>
                        <span className="text-gray-800 font-bold">2 orang</span> {/* Placeholder porsi */}
                    </div>
                    <div className="bg-white rounded-2xl p-6 flex flex-col items-center justify-center shadow-sm border border-gray-100 gap-2">
                        <Flame className="w-8 h-8 text-green-500 mb-1" />
                        <span className="text-gray-400 text-xs uppercase tracking-wider font-semibold">Tingkat</span>
                        <span className="text-gray-800 font-bold capitalize">{resepDetail.tingkat_kesulitan}</span>
                    </div>
                </div>

                {/* Actions */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <button className="flex items-center justify-center gap-2 bg-orange-400 hover:bg-orange-500 text-white py-3.5 px-6 rounded-xl font-semibold transition-colors shadow-sm shadow-orange-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 h-fit">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b border-gray-100 pb-4">Bahan-bahan</h2>
                            ...
                            {resepDetail.bahan.map((bahan, index) => (
                                <li key={index} className="flex items-start gap-3 group">
                                    ...
                                    <div className="text-gray-600 group-hover:text-gray-900 transition-colors">
                                        <span className="font-medium text-gray-800">{bahan.takaran} {bahan.satuan}</span> 
                    ...
                    {/* Steps */}
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 h-fit">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b border-gray-100 pb-4">Cara Membuat</h2>
